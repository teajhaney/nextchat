...
export const useMessageStore = create<MessageState>((set, get) => ({
  ...
  //UPDATE message in the store (NEW METHOD - add this)
  updateMessage: (updatedMessage: Message) => {
    ...
    const isCurrentChat =
      ...
      (updatedMessage.sender_id === selectedChatUser.id && updatedMessage.recipient_id === user.id);
  ...
  // In your message store, remove the markConversationAsRead method entirely
  // And modify the markMessagesAsRead to be simpler:
  markMessagesAsRead: async (messageIds: string[]) => {
    const { messages, selectedChatUser } = get();
    if (!selectedChatUser || messageIds.length === 0) return;
